...
// The cushion of each style comes from shared/seats.ts, which is also where the seated avatar's
// height is derived from — so the two can never disagree.
const C = CUSHIONS;
...
function GamingChair() {
  ...
  return (
    <>
      ...
      <Box p={[0, C.gaming.y, 0]} s={[0.46, C.gaming.h, 0.46]} m={seat} />
...
function LogSeat() {
  ...
  return (
    <>
      <Cylinder p={[0, C.log.y, 0]} s={[C.log.h, 1.2, C.log.h]} r={[0, 0, Math.PI / 2]} m={M.log} />
      {[-0.6, 0.6].map((x) => (
        <Cylinder key={x} p={[x, C.log.y, 0]} s={[C.log.h - 0.04, 0.01, C.log.h - 0.04]} r={[0, 0, Math.PI / 2]} m={M.logEnd} />
      ))}
    </>
  );
}
...
function BarStool() {
  // The padded top is CUSHIONS.stool; the stool's sitY in shared/props.ts is derived from it.
  return (
    <>
      {[
        [-0.16, -0.16],
      ...
      <Cylinder p={[0, C.stool.y - 0.05, 0]} s={[0.44, 0.07, 0.44]} m={M.walnut} />
      <Cylinder p={[0, C.stool.y, 0]} s={[0.4, C.stool.h, 0.4]} m={M.cushion} />
    </>
  );
}
...
function Armchair() {
  return (
    <>
      ...
      <Box p={[0, C.armchair.y, 0.06]} s={[0.6, C.armchair.h, 0.64]} m={M.velvet} />
...
function WoodChair() {
  return (
    <>
      ...
      <Box p={[0, C.wood.y, 0]} s={[0.5, C.wood.h, 0.5]} m={M.oak} />
...
function DeckChair() {
  ...
  return (
    <>
      ...
      {/* the sling: its middle (where the hips land) is CUSHIONS.deckchair */}
      <Box p={[0, C.deckchair.y + 0.03, 0.12]} s={[0.56, C.deckchair.h, 0.7]} m={M.canvasA} r={[0.25, 0, 0]} />
