...
export function WorldDrawer({ currentMap, playerCount, disabled, onSelect, onClose }: { currentMap: MapId; playerCount: number; disabled: boolean; onSelect: (m: MapId) => void; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50" onPointerDown={(e) => e.target === e.currentTarget && onClose()} role="presentation">
      <aside className="clay-slide-left font-cozy absolute left-0 top-0 flex h-full w-[min(560px,96vw)] flex-col gap-3 overflow-y-auto border-r border-white/10 bg-stone-900/85 p-4 pt-[max(16px,env(safe-area-inset-top))] text-stone-100 shadow-[12px_0_40px_rgba(0,0,0,0.5)] backdrop-blur-md" aria-label="Fast travel">
