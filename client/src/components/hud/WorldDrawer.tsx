...
export function WorldDrawer({ currentMap, playerCount, disabled, onSelect, onClose }: { currentMap: MapId; playerCount: number; disabled: boolean; onSelect: (m: MapId) => void; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50" onPointerDown={(e) => e.target === e.currentTarget && onClose()} role="presentation">
      <aside className="clay-slide-left font-cozy absolute left-0 top-0 flex h-full w-[min(320px,88vw)] flex-col gap-3 overflow-y-auto border-r border-white/10 bg-stone-900/85 p-4 pt-[max(16px,env(safe-area-inset-top))] text-stone-100 shadow-[12px_0_40px_rgba(0,0,0,0.5)] backdrop-blur-md" aria-label="Fast travel">
        ...
        {MAP_IDS.map((id) => {
          ...
          return (
            ...
            >
              <div className={`relative h-24 bg-gradient-to-b ${art.sky}`}>
                ...
                {art.props.map((emoji, i) => (
                  <span key={emoji} className="absolute text-2xl drop-shadow" style={{ left: `${12 + i * 22}%`, bottom: `${10 + (i % 2) * 14}px`, transform: `rotate(${(i - 1.5) * 4}deg)` }}>
                    {emoji}
                  </span>
                ))}
                {here && <span className="absolute right-2 top-2 rounded-full bg-amber-300 px-2 py-0.5 text-[11px] font-extrabold text-amber-950">You are here</span>}
              </div>
              <div className="flex items-center gap-3 px-4 py-3">
                <span className="text-2xl">{label.icon}</span>
                <span className="flex-1">
                  <span className="block text-base font-extrabold">{label.name}</span>
                  <span className="block text-xs opacity-70">{label.tagline}</span>
                </span>
                <span className={`rounded-full px-2 py-0.5 text-xs font-bold ${here ? "bg-emerald-400/25 text-emerald-200" : "bg-white/10 opacity-70"}`}>👥 {here ? playerCount : 0}</span>
              </div>
            </button>
          );
        })}
