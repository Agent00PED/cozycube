    ...
    <Modal title="Jukebox" icon="🎵" onClose={onClose} width={400}>
      <div className="flex flex-col gap-2 pb-2">
        ...
        {RECORD_NAMES.map((r, i) => (
          <button key={r.name} type="button" onClick={() => (onPick(i))} className={`flex min-h-14 items-center gap-3 rounded-2xl px-4 text-left transition-transform active:scale-[0.98] ${playing === i ? "bg-amber-300/25 ring-2 ring-amber-300/60" : "bg-white/5 hover:bg-white/10"}`}>
        ...
        <button type="button" className="clay-btn clay-btn-ghost" onClick={() => (onPick(-1))}>
          ⏏ Lift the needle
        </button>
      </div>
    </Modal>
